...
pub fn sphere(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = params[0] * u.cos() * v.sin();
    let y = params[0] * u.sin() * v.sin();
    let z = params[0] * v.cos();

    [x, y, z]
}

pub fn plane(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    [u, v, params[0]]
}

pub fn cone(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = params[0] * (1.0 - v) * u.cos();
    let y = params[0] * (1.0 - v) * u.sin();
    let z = params[1] * v;

    [x, y, z]
}

pub fn hyperbola(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = params[0] * u.cosh() * v.cos();
    let y = params[0] * u.cosh() * v.sin();
    let z = params[1] * u.sinh();

    [x, y, z]
}

pub fn mobius_strip(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = (params[0] + v * (u / 2.0).cos()) * u.cos();
    let y = (params[0] + v * (u / 2.0).cos()) * u.sin();
    let z = v * (u / 2.0).sin();

    [x, y, z]
}

pub fn ellipsoid(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = params[0] * u.cos() * v.sin();
    let y = params[1] * u.sin() * v.sin();
    let z = params[2] * v.cos();

    [x, y, z]
}

pub fn cylinder(u: f32, v: f32, params: [f32; 5]) -> [f32; 3] {
    let x = params[0] * u.cos();
    let y = params[0] * u.sin();
    let z = v;
...
pub fn peaks(x: f32, z: f32) -> [f32; 3] {
    let y = 3.0 * (1.0 - x).powi(2) * (-x.powi(2) - (z + 1.0).powi(2)).exp()
        - 10.0 * (x / 5.0 - x.powi(3) - z.powi(5)) * (-x.powi(2) - z.powi(2)).exp()
        - 1.0 / 3.0 * (-(x + 1.0).powi(2) - z.powi(2)).exp();
    [x, y, z]
}
